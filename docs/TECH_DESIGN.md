...
| Sync/async | async (`pymongo.AsyncMongoClient`, per ADR-0002) — specifically because a stalled Atlas index can block ~30s and would otherwise freeze the uvicorn event loop | **async as of `cms-hte-patient-matching` 0.0.5** (its own PR #48) — `MongoAtlasCache`'s per-rule round-trips were exactly this same event-loop-blocking risk, so the whole call chain (`CacheBackend`, `MatchingBackend`, `MatchingEngine.match`, `PatientMatcherService.match_patient`/`match_from_token`, `FhirClient`, `TokenVerifier`) converted to `async def`. `DuckDBCache` implements the same async interface even though it has no real I/O to yield on, for one uniform `CacheBackend` contract. `MatchingEngine.evaluate_pair()` and `patient_matching.normalization` stay sync (pure computation, no I/O). This service's `MatchController.match()` and `_build_cache()` now `await` accordingly. |
...
2. **Add the package dependency**: declare `cms-hte-patient-matching` (importable as `patient_matching`) in `pyproject.toml`. **Resolved 2026-09-05**: published to public PyPI — https://pypi.org/project/cms-hte-patient-matching/ (v0.0.2 as of this writing). No JFrog-side publish step needed: bWell's JFrog `virtual-pypi` index already proxies public PyPI (confirmed in `person-matching-service`'s own index config), so `uv add cms-hte-patient-matching` should resolve through the existing index with no additional platform-team action. Verify the resolved version once actually run against this repo's `uv sync` before assuming this is fully unblocked — the PyPI description still says "26 approved Table 2 rules" (the v3.2.2-era count), so confirm which rule-set version (26 vs. the v3.3 38-rule expansion, in flight as `cms-hte-patient-matching` PR #39) v0.0.2 actually ships before depending on rule-count-sensitive behavior.
